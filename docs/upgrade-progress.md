...
2026-08-28 功能测试补齐:V2 回测 API(TestClient+真引擎)与七-agent 流水线全链贯通测试落地;抓出并修复 3 个真 bug(report_agent 的 await/方法名两处——合并后流水线报告路径实际已断、^GSPC 基准被股票校验误拦);pipeline 结果视图补齐 V2 渲染(周线/估值情景/红旗/委员会卡)与 confidence 0-1 适配。回测 UI 调旧 /run,已由 V2 引擎支撑,无需改动。当前 270 测试。
