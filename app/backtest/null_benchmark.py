...
def _place_segments(
    ...
    """Scatter the holding lengths over random non-overlapping start bars.

    Segment order is shuffled per draw so a segment is equally likely to land
    anywhere; a placement that cannot fit after PLACEMENT_ATTEMPTS tries is
    dropped (the draw simply has one fewer round trip).
    """
    occupied = np.zeros(n_bars, dtype=bool)
    placed: list[tuple[int, int]] = []
    for length in rng.permutation(lengths):
        length = int(length)
        # Full placement freedom, including holding into the final bar —
